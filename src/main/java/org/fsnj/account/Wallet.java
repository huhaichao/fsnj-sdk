...
public class Wallet {
    ...
    public Account createAccount() throws InvalidAlgorithmParameterException, NoSuchAlgorithmException, NoSuchProviderException {
     ...
     */
    public Account createAccount(String seed) throws NoSuchAlgorithmException {
        ECKey ecKey = ECKey.fromPrivate(generatePriKey(seed));
    ...
    public static String generateSeed() {
        ...
        byte[] rhash = HashUtil.hash256(bytes, 0, 20);
        bytes[20] = rhash[0];
    ...
    public  byte[] generatePriKey(String accountSeed) {
        ByteBuffer buf = ByteBuffer.allocate(accountSeed.getBytes().length);
        buf.put(accountSeed.getBytes());
        byte[] accountSeedByte = HashUtil.hash256(buf.array(), 0, buf.array().length);
        accountSeedByte = HashUtil.sha3(accountSeedByte);
        byte[] hashedSeed = HashUtil.hash256(accountSeedByte, 0, accountSeedByte.length);
